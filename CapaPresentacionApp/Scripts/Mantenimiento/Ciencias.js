...
tablaData = $("#tablaLibros").DataTable
    ({
        ...
        ordering: false,
        "ajax": {
            url: '/Mantenimiento/ListarCiencias', //@Url.Action("ListarCiencias", "Mantenimiento")
            type: "GET",
            dataType: "json",
        },
        "columns":
...
function Guardar() {
    ...
    jQuery.ajax({
        url: '/Mantenimiento/GuardarCiencias', //@Url.Action("GuardarEditoras", "Mantenimiento")
        type: "POST",
        data: JSON.stringify({ Ciencias: Ciencias }), // parametro del metodo GuardarUsuario que es usuariox y se carga con el objeto Usuario
        dataType: "json",
        contentType: "application/json; charset=utf-8",
        success: function (data) { //la data es lo que resivimos del url que viene del controlador metodo GuardarUsuario

            if (Ciencias.Id == 0) { // si usuarioID = 0 es que se va a agregar
                ...
                if (data.resultado) {
                    ...
                    /*tablaData.row(filaSeleccionada).data(Ciencias).draw(false);*/
